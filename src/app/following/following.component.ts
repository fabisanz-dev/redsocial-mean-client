...
export class FollowingComponent implements OnInit {
  ...
  public followingContent : Follow []; //propiedad que contiene el objeto de follow
  ...
  ngOnInit() {
    console.log('componente siguiendo corriendo')
    this.page = 1;
    this.ActualPage();
  }
  ...
  ActualPage(){
    ...
    this._route.params.subscribe(
      params =>{
         ...
         //llamar al metodo de listado de usuarios siguiendo paginados
         this.getFollowing(this.userSearchId, this.page);
  ...
  getFollowing(idUser, page){
    this._followService.followingUserList(idUser, page).subscribe(
      response =>{
        if(!response){
          this.status = 'error';
        }else{
          this.followingContent = response.following;
          this.total = response.total;
          this.pages = response.pages;

          //console.log(this.followingContent)
  ...
  this._followService.addFollow(this.follow).subscribe(
    result =>{
      if (!result && !result.follow.followed) {
        ...
        //agrego el id del nuevo usuario al que seguira en el array following y actualiza la pagina
  ...
  this._followService.deleteFollow(idUserFollowing).subscribe(
    result =>{
      ...
      if(result.message == null){
        ...
        this.updateSidebarFollow = 'delFollowing'; //avisar al sidebar el estado y actualizar el stat
